...
class H_functions:
    ...
    def Ht(self, vec):
        ...
        temp = self.Ut(vec)
        singulars = self.singulars()
        return self.V(self.add_zeros(temp[:, :singulars.shape[0]], singulars))
...
class Inpainting(H_functions):
    ...
    def add_zeros(self, vec):
        ...
        for i in range(self.batch_size):
            temp[i, :self._singulars[i].shape[0]] = vec[i, :self._singulars[i].shape[0]]
    ...
    def Lambda(self, vec, a, sigma_y, sigma_t, eta):
        ...
        for i in range(self.batch_size):
            ...
            temp_singulars[:singulars[i].size(0)] = singulars[i]
            final_singulars = temp_singulars
            inverse_singulars = 1. / final_singulars
            inverse_singulars[final_singulars == 0] = 0.

            if a != 0 and sigma_y != 0:
                change_index = (sigma_t < a * sigma_y * inverse_singulars) * 1.0
                lambda_t = lambda_t * (-change_index + 1.0) + change_index * (
                        final_singulars * sigma_t * (1 - eta ** 2) ** 0.5 / a / sigma_y)

            lambda_t = lambda_t.reshape(1, -1)
            lambda_ts.append(lambda_t)
            
        out = out * torch.stack(lambda_ts).squeeze().to(vec.device)

        result = torch.zeros_like(temp)
        
        for i in range(self.batch_size):
            result[i, self.kept_indices[i]] = out[i, :self.kept_indices[i].shape[0]]
            result[i, self.missing_indices[i]] = out[i, self.kept_indices[i].shape[0]:]
    ...
    def Lambda_noise(self, vec, a, sigma_y, sigma_t, eta, epsilon):
        ...
        for i in range(self.batch_size):
            out_vec[i, :self.kept_indices[i].shape[0]] = temp_vec[i, self.kept_indices[i]]
            out_vec[i, self.kept_indices[i].shape[0]:] = temp_vec[i, self.missing_indices[i]]
        ...
        for i in range(self.batch_size):
            out_eps[i, :self.kept_indices[i].shape[0]] = temp_eps[i, self.kept_indices[i]]
            out_eps[i, self.kept_indices[i].shape[0]:] = temp_eps[i, self.missing_indices[i]]
        ...
        for i in range(self.batch_size):
            ...
            if a != 0 and sigma_y != 0:
                ...
                change_index = (final_singulars == 0) * 1.0
                d1_t = d1_t * (-change_index + 1.0) + change_index * sigma_t * eta
        ...
        out_vec = out_vec * torch.stack(d1_ts).squeeze().to(vec.device)
        out_eps = out_eps * torch.stack(d2_ts).squeeze().to(vec.device)

        result_vec = torch.zeros_like(temp_vec)

        for i in range(self.batch_size):
            result_vec[i, self.kept_indices[i]] = out_vec[i, :self.kept_indices[i].shape[0]]
            result_vec[i, self.missing_indices[i]] = out_vec[i, self.kept_indices[i].shape[0]:]
        ...
        for i in range(self.batch_size):
            result_eps[i, self.kept_indices[i]] = out_eps[i, :self.kept_indices[i].shape[0]]
            result_eps[i, self.missing_indices[i]] = out_eps[i, self.kept_indices[i].shape[0]:]
