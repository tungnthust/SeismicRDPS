...
def svd_based_ddnm_plus(model, diffusion, measurement, mask, num_sampling_steps, device):
    batch_size, channels, _, image_size = measurement.shape
    mask = mask.reshape(batch_size, -1)
    missing = torch.stack([torch.nonzero(mask[i] == 0) for i in range(batch_size)]).long().reshape(batch_size, -1)
    A_funcs = Inpainting(channels, image_size, missing, device)
    
    
    sigma_y = SIGMA_Y
    eta = ETA
    y = measurement.reshape(batch_size, -1)
    Apy = A_funcs.H_pinv(y).view(batch_size, channels, image_size, image_size)
    Apy += A_funcs.H_pinv(A_funcs.H(torch.ones_like(Apy))).reshape(*Apy.shape) - 1
    # init x_T
        ...
        for i, j in tqdm(time_pairs):
            ...
            if j < i: # normal sampling 
               ...
               # Eq. 12
                ...
                # Eq. 17
                x0_t_hat = x0_t - A_funcs.Lambda(A_funcs.H_pinv(
...
def simplified_based_ddnm(model, diffusion, measurement, mask, num_sampling_steps, device):
    A = lambda z: z*mask
    Ap = A
