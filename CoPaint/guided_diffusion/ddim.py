        ...
        **kwargs,
        ...
        if self.mode == "inpaint":
            def loss_fn(_x0, _pred_x0, _mask):
                ...
                ret = torch.linalg.norm(_x0 * _mask - _pred_x0 * _mask, dim=(2,3))
                return ret
        ...
        def reg_fn(_origin_xt, _xt):
            ...
            ret = torch.linalg.norm(_origin_xt - _xt, dim=(2,3))
            return ret
        ...
        with torch.enable_grad():
            ...
            prev_loss = loss_fn(x0, pred_x0, mask)
            ...
            for step in range(self.num_iteration_optimize_xt):
                ...
                x_grad = torch.autograd.grad(
                    loss, x, grad_outputs=grad_outputs, retain_graph=False, create_graph=False
                )[0].detach()
                ...
                while self.use_adaptive_lr_xt and True:
                    with torch.no_grad():
                        ...
                        if not torch.isnan(new_loss).sum() and torch.sum(new_loss <= loss):
                            break
                        else:
                            lr_xt *= 0.8
                            # logging_info(
        ...
        # after optimize
        with torch.no_grad():
            new_loss = loss_fn(x0, pred_x0, mask).mean()
            # logging_info("Loss Change: %.3lf -> %.3lf" % (prev_loss, new_loss))
            new_reg = reg_fn(origin_x, new_x).mean()
