...
def get_metric(gt, pred):
    ...
    ssim_score = structural_similarity(np.array(gt)[0, 0], np.array(pred)[0, 0])
    psnr_score = peak_signal_noise_ratio(np.array(gt)[0, 0], np.array(pred)[0, 0])
...
def main():
    ...
    for idx, img in enumerate(dataset):
        ...
        #     print(f"{n_sample}: SSIM: {v[0]} - PSNR: {v[1]} - SNR: {v[2]}")
